...
| infer the system | Egger et al. 2024, A&A (arXiv:2406.18653) | HIP 29442 on the mass–radius diagram with the TESS-versus-CHEOPS radii; plaNETic's full-grid acceptance-rejection run live with the paper's priors (water accreted as solids or from gas, three Si/Mg/Fe options, mantle-composition rejection) on a Lopez–Fortney/Zeng forward model; posterior interiors per birthplace; the paper's CHAIN escape rates with stripping times; the toy Kubyshkina engine's verdict per planet and the Bern-model formation ledger |
| read the architecture | Leleu et al. 2021, A&A 649, A26 (arXiv:2101.09260) | TOI-178: a synthetic TESS sector from the real ephemerides with a live box-least-squares search that reproduces the 10.35-day f+g ghost; an in-browser N-body integration (heliocentric kick–drift–kick, exact Kepler drifts) of the 2:4:6:9:12 Laplace chain with perturbation sliders and a diffusion index; six-year transit timing variations with mass scaling; densities against temperature and the mass–radius diagram with the toy interior sampler run on all six planets; the coplanarity fit, the 26-day transit limit and the eq. C.7 continuation periods |
| classify the system | Mishra et al. 2023, A&A 670, A68 (arXiv:2301.02374) | the two architecture coefficients computed live for real systems and slider-built ones, with the telescoping identity made visible; a Monte Carlo of the architecture plane with the Appendix C tolerance limits (plus the exact variation cap, which exceeds the paper's formula from six planets up); the 41 observed systems of Table 2 and the class frequencies against the Bern model; a radial-velocity survey toy that shows detection flipping a system's class; radius-versus-mass architecture and habitable-zone hosting by class |
| predict the Earth | Davoult et al. 2024, A&A (arXiv:2408.12251) | the Earth-like definition and the radial-velocity floor it implies, recomputed live for three stellar types; PCA architecture classes with the ellipse drawn on real systems and compared with Mishra's; theoretical-versus-biased class shares and Earth-hosting fractions from Tables 4 to 7; the full pipeline on real systems, floor, survivors, class, innermost detectable planet and the Table 8 conditional probabilities against whether the system really hosts one; the paper's list of most promising signatures and its caveats |
| the clock on every planet | Planet evolution (concept tab, after Alibert) | one planet under a star that is itself evolving: Baraffe tracks for 0.3/0.6/1.0 M☉ with rotation-dependent X-ray histories, the Alibert cross-section shrinking by cooling (Lopez & Fortney tables) and thinning by escape with one time knob, a 400-planet population whose radius valley opens by itself on the mass–radius and period–radius diagrams over the real archive, and the same planet around three stars at equal temperature today |
| the fidelity ladder | Changing periods (concept tab, after Leleu) | every way an orbital period changes, one rung per station: Kepler's fixed period; the precessing ellipse and its three periods (relativity, stellar J₂, tidal bulge, a neighbour); a second planet integrated live with near-resonant swings, libration and chaos; tidal decay (WASP-12b), resonant repulsion and mass loss; light-travel time and Kozai cycles from a distant companion; type I migration with resonance capture; a closing timescale-versus-size chart and signature table |
...
All nine papers of the reading list now have a tab.
